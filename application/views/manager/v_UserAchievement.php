...
<script>
	  ...
	  var myTable;
	  var topic_name = "log_achievement";// edit this
	  ...
	  var url_get_option_role = "<?php echo site_url("Source_manager/get_data/");?>roles";
	  var url_get_option_achievement = "<?php echo site_url("Source_manager/get_data/");?>achievement";
     ...
     */
	 function set_Data(){
		...
		$.ajax({
			...
			success: function(a){
				for(var i=0 ;i<a.length;i++){
					Options_achievement[a[i].id] = "(Point : "+a[i].point+") "+a[i].name;			
				}
			},
			error: function(){
			}
		...
		var columnDefs = [
					...
					},
					{
						...
						select2 : { width: "100%"},
						render: function (data, type, row, meta) {
							if (data == null || !(data in Options_achievement)) return null;
							return Options_achievement[data];
						}
					},
				];
