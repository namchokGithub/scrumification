...
<div class="panel panel-primary">
    <div  class="panel-heading" style=" font-size: 28px; ">Activity Cluster <?php echo $id; ?></div>
	...
	<div style="font-size: x-large;font-weight: 600; text-align: right;">
		<select id="select-opt">
		  ...
		  <?php for($i=2;$i<12;$i++){?>
		  <option value="<?php echo site_url('mainreward/index/'.$i); ?>">Cluster <?php echo $i-2; ?></option>
		  <?php } ?>
		</select>
	</div>
		<div id="clock"></div>
		...
		"><span style="
				left: 0px;
				position: absolute;
			">ชั่วโมง</span><span style="
				left: 135px;
				position: absolute;
			">นาที</span><span style="
				left: 235px;
				position: absolute;
			">วินาที</span></div>
		    ...
		    <div style="padding: 0px 30px 0px 30px;">
			  ...
			  <div class="small-box bg-gray" style=" padding: 15px; border-radius: 25px; ">
				<div class="inner">
				  ...
				  <h3 style="color: black !important;width: 100%;overflow: hidden;text-overflow: ellipsis;font-family:prompt" id="name"><?php echo $row["name"] ?></h3>

				  <p style="color: black !important;font-size:20px" id="time"><?php echo $row["time_start"] ?> - <?php echo $row["time_end"] ?></p>
				</div>
				<div class="icon">
            ...
            <div class="box-body no-padding" style=" overflow-x: auto; overflow-y: hidden; ">
              <table class="table table-condensed mobile-table" style=" margin: 0px 4% 0px 4%; width: -webkit-fill-available; ">
				...
				<tr style="border-bottom: solid skyblue;" class="row_data">
                  ...
                  <td onclick="checkin(<?php echo $row["id"] ?>)" style=" cursor: pointer; "><small class="label label-warning"><i class="fa fa-clock-o"></i> Wait</small></td>
...
function checkin($user_id){
	 $.get("<?php echo site_url("mainreward/add_Activity/"); ?>"+targle_input+"/"+$user_id, function(data, status){
	 })
	 .done(function(data) {
...
function clear_get_data(){
    $.get("<?php echo site_url("mainreward/get_Activity/"); ?>"+targle_input+"/<?php echo $id+2;?>", function(data, status){
...
setInterval(function(){ 
	today = new Date()
	$(".owl-item").find(".small-box").each(function(){$(this).attr("class","small-box bg-gray")})
