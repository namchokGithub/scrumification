...
<div class="panel panel-primary">
    <div  class="panel-heading heading-targle" style=" font-size: 28px;display:flex; cursor:pointer "  aria-hidden="true" data-toggle="collapse" data-target="#Panel_Challenger" >Challenger<div style="margin-left: auto;"><i class="fa fa-chevron-down" id="collapse_icon" ></i></div></div>
	...
	<?php for($i=0;$i<10;$i++){?>
		<div class="<?php echo ((1==($i+1)%5)?" col-lg-offset-1 ":" "); ?>col-lg-2 col-md-4 col-sm-4 col-xs-6 card_group" id="<?php echo $i+2;?>" >
			<div class="card" >
				...
				<div class="box-footer bg-maroon card-header-font" style="background-color:#3282b8  !important;text-align: center;padding-top: 0px;border-top: 0px !important">Cluster <?php echo $i; ?><!-- /.row -->
				</div>
 ...
 */
  $(".mainreward").click(function(){
		...
		if(action){
			$('html,body').animate({scrollTop: $('#card_reward').offset().top-75},1000);
		}
		action++;
		$(".info-box-text").each(function(){
			$(this).closest('#card_reward').attr('class', 'info-box bg-red');
		...
		$.get("<?php echo site_url("Home/get_Activity/"); ?>"+targle_id, function(data, status){
		...
		$.get("<?php echo site_url("Home/get_all_Achievement/"); ?>"+targle_id, function(data, status){
			var raw_data = JSON.parse(data);
			console.log(raw_data)
...
<div class="panel panel-primary">
    <div class="panel-heading"  style=" font-size: 28px; ">Main Reward of Cluster <span class="cluster_title">0</span></div>
			...
			<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 card_reward" id="<?php echo "2/".$key;?>">
			  <div id="card_reward" class="info-box bg-yellow" style=" cursor: pointer; ">
				...
				<div class="info-box-content">
				  ...
				  <span class="info-box-number"><?php echo $number_new;?>/100 <span style=" position: absolute; right: 20px; ">0%</span></span>

				  <div class="progress">
					<div class="progress-bar" style="width: <?php echo $number_new;?>%"></div>
				  </div>
					  <span class="progress-description">
						<?php echo $row["time_start"] ?> - <?php echo $row["time_end"] ?>
					  </span>
				</div>
				<!-- /.info-box-content -->
			  </div>
...
<div class="panel panel-primary">
    <div class="panel-heading"  style=" font-size: 28px; ">Achievement of Cluster <span class="cluster_title">0</span></div>
    <div class="panel-body">
