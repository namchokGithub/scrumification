    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                this.language.modalClose = this.language.modalClose || 'ยกเลิก';
                this.language.edit = this.language.edit || {};
                this.language.edit = { title: this.language.edit.title || 'หน้าแก้ไขข้อมูล',
                                       button: this.language.edit.button || 'บันทึก'
                                     };
                this.language.delete = this.language.delete || {};
                this.language.delete = { title: this.language.delete.title || 'หน้าลบข้อมูล',
                                         button: this.language.delete.button || 'บันทึก' };
                this.language.add = this.language.add || {};
                this.language.add = { title: this.language.add.title || 'หน้าเพิ่มข้อมูล',
                                      button: this.language.add.button || 'บันทึก'
                                    };
                this.language.success = this.language.success || 'ดำเนินการเสร็จสิ้น';
                ...
                this.language.error = { message: this.language.error.message || 'There was an unknown error!',
                                        label: this.language.error.label || 'เกิดข้อผิดพลาด!',
                ...
                var modal = '<div class="modal fade" id="' + modal_id + '" tabindex="-1" role="dialog">' +
                    ...
                    '<button style="margin: initial;opacity:0.75" type="button" class="close" data-dismiss="modal" aria-label="' + this.language.modalClose + '">' +
                    '<span aria-hidden="true" style=" font-size: 34px; font-weight: 700; ">&times;</span></button>' +
                    '</div>' +
            ...
            _openDeleteModal: function () {
				...
				for (var j in columnDefs) {
					...
					$(selector).find(jquerySelector).val(this._quoteattr(selectedValue));
					$(selector).find(jquerySelector).trigger("change"); // required by select2
					var span_text = "$('"+selector+"').find('"+jquerySelector+'\').val('+this._quoteattr(selectedValue)+')'
				}
                $(selector).on('show.bs.modal', function () {
                    var btns = '<button type="button" data-content="remove" class="btn btn-danger" data-dismiss="modal"  style="margin-right: 75%;">' + that.language.modalClose + '</button>' +
                        '<button type="submit"  data-content="remove" class="btn btn-success" id="deleteRowBtn">' + that.language.delete.button + '</button>';
            ...
            */
            createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
                    ...
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        if (columnDefs[j].type.indexOf("readonly") >= 0) {
                            ...
                            data += "<input type='text' readonly  id='"
                                ...
                                + "' placeholder='"
                                + this._quoteattr(columnDefs[j].title)
                                + "' style='overflow:hidden'  class='form-control  form-control-sm' value=''"+disabled_text+">";
                        }
                ...
                var selector = this.modal_selector;
                $(selector).on('show.bs.modal', function () {
                    var btns = '<button type="button" data-content="remove" class="btn btn-danger " data-dismiss="modal" style="margin-right: 75%;">'+closeCaption+'</button>' +
                        '<button type="submit" form="' + formName + '" data-content="remove" class="btn btn-success" id="'+buttonClass+'">'+buttonCaption+'</button>';
...
//console.log(rowDataArray); //DEBUG
            ...
            _deleteRowCallback: function (response, status, more) {
					...
					$('.modal').modal('hide');
					toastr["success"](this.language.success)
            },
            ...
            _errorCallback: function (response, status, more) {
                    ...
                    $(selector + ' .modal-body').append(message);
					$('.modal').modal('hide');
					toastr["error"]( '<strong>' + this.language.error.label + '</strong> ' + (error.status == null ? "" : this.language.error.responseCode + error.status) + " " + errstr)
            },
